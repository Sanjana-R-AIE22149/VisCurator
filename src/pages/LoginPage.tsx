...
import { apiLogin, clearToken, getToken, setToken } from '../lib/api';
...
export default function LoginPage() {
  ...
  const [password, setPassword] = useState('viscurator');
  ...
  const handleSsoClick = (provider: 'GitHub' | 'Google') => {
    setError('');
    setNotice(`${provider} sign-in is not configured for this local CVAgent build. Use the username and password fields above.`);
  };
          ...
          <div className="mt-4 flex gap-2">
            {['GitHub', 'Google'].map((provider) => (
              <button
                ...
                type="button"
                onClick={() => handleSsoClick(provider as 'GitHub' | 'Google')}
