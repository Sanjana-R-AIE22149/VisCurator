...
import {
  ...
  TrendingUp, AlertCircle, Download, ArrowRight, Cpu,
} from 'lucide-react';
...
export default function AnnotatorPage() {
  ...
  useEffect(() => { document.title = 'Annotator — VisCurator'; }, []);
          ...
          <div className="space-y-4">
            ...
            {(phase === 'done' || phase === 'aug_done') && (
              <div className="space-y-2">
                ...
                {/* ── Next Step CTA ── */}
                {phase === 'done' && (
                  <div className="flex items-center gap-4 rounded-xl border border-violet-500/30 bg-gradient-to-r from-violet-500/10 to-violet-500/5 px-4 py-3">
                    <Wand2 className="w-5 h-5 text-violet-400 shrink-0" />
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-violet-300">Step 2 complete — annotation done</p>
                      <p className="text-xs text-slate-500">Use the Augmentation Agent to multiply your dataset with class-aware transforms.</p>
                    </div>
                    <button
                      id="cta-go-augment"
                      onClick={() => navigate('/augmentation')}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold transition-all shadow-lg shadow-violet-500/20 shrink-0"
                    >
                      Augment <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                  </div>
                )}
                {phase === 'aug_done' && (
                  <div className="flex items-center gap-4 rounded-xl border border-teal-500/30 bg-gradient-to-r from-teal-500/10 to-teal-500/5 px-4 py-3">
                    <Cpu className="w-5 h-5 text-teal-400 shrink-0" />
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-teal-300">Step 3 complete — dataset augmented</p>
                      <p className="text-xs text-slate-500">Design a model architecture in the Builder and launch a training run.</p>
                    </div>
                    <button
                      id="cta-go-train"
                      onClick={() => navigate('/builder')}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold transition-all shadow-lg shadow-teal-500/20 shrink-0"
                    >
